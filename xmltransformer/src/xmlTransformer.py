    ...
    def tranformation(self):
        if self.input_xml.endswith('txt'):
            ...
            for i in input_list:
                ...
                print("Transforming {} to {}_output.xml".format(i, self.xsl, self.output_xml))
                #os.system("java net.sf.saxon.Transform -s:{0} -xsl:{1} -o:{2}_output.xml".format(i,self.xsl,i.split('.')[0]))
                os.system("java net.sf.saxon.Transform -s:{0} -xsl:{1} -o:{2}_output.xml".format(i, self.xsl,self.output_xml))
            ...
            print("Transforming {} to {}_output.xml".format(self.input_xml, self.xsl, self.output_xml))
            #os.system("java net.sf.saxon.Transform -s:{0} -xsl:{1} -o:{2}_output.xml".format(self.input_xml,self.xsl,self.input_xml.split('.')[0]))
            os.system("java net.sf.saxon.Transform -s:{0} -xsl:{1} -o:{2}_output.xml".format(self.input_xml, self.xsl,self.output_xml))
